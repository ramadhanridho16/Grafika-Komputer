...
double angle = 0.0, yAngle = 0.0;
// actual vector representing the camera's direction
double lx = 0.0f, ly = 0.0f, lz = -1.0f;
// XZ position of the camera
double x = -5.0f, z = 18.0f;
double roll = 0.0f;
...
void interactWithSnowman() {
	...
	char str1[] = "HALO, Ramadhan Ridho Arrohman !!!";
	int l1 = strlen(str1); // see how many characters are in text string.
	glPushMatrix();

	glColor3f(0.0f, 0.0f, 0.0f);

	glRasterPos3f(-9.6f, 2.7f, -2.8f); // location to start printing text
	for (int i = 0; i < l1; i++) // loop until i is greater then l
	{
		glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, str1[i]); // Print a character on the screen
	}
	glPopMatrix();
}
...
void renderScene() {
	...
	glBegin(GL_QUADS);
	glVertex3f(-10.0f, 0.0f, -10.0f); // Tembok arah utara, nilai z tidak ada yang berbeda.
	...
	glBegin(GL_LINES);
	glVertex3f(-9.99f, 0.0f, -9.99f);
	glVertex3f(-9.99f, 7.0f, -9.99f);
	glEnd();
	...
	glBegin(GL_QUADS);
	glVertex3f(-10.0f, 0.0f, -10.0f); //Tembok arah barat nilai x berupa bilangan negatif.
	glVertex3f(-10.0f, 7.0f, -10.0f);
	glVertex3f(-10.0f, 7.0f, 10.0f);
	glVertex3f(-10.0f, 0.0f, 10.0f);
	glEnd();
	...
	glColor3f(0.98f, 0.98f, 0.98f);
	glBegin(GL_QUADS);
	glVertex3f(10.0f, 0.0f, -10.0f); // Tembok arah timur, nilai x berupa bilangan positif
	glVertex3f(10.0f, 7.0f, -10.0f);
	glVertex3f(10.0f, 7.0f, 10.0f);
	glVertex3f(10.0f, 0.0f, 10.0f);
	glEnd();
	...
	// Draw prof's chair
	Chair profChair;   //Membuat object dari Chair Class
	glPushMatrix();
	glTranslatef(6.0f, 1.0f, -9.0f);
	glScalef(0.32f, 0.32f, 0.32f);
	glRotatef(-30.0, 0.0, 1.0, 0.0);
	profChair.drawChair();   //Memanggil metode yang dibuat oleh objek profChair.
	glPopMatrix();
	...
	if (abs(mouseX) > 0.3) {
		angle -= (0.0012054443359375 * mouseX);
		lx = sin(angle);
		lz = -cos(angle);
	}
	if (abs(mouseY) > 0.3) {
		if (abs(yAngle) < (M_PI / 2)) {
			yAngle += (0.0007171630859375 * mouseY);
		}
		ly = sin(yAngle);
	}
...
void processMouseMovement(int xx, int yy) {
	...
	angle -= (0.0014495849609375 * mouseX);
	lx = sin(angle);
	lz = -cos(angle);

	if (abs(yAngle) < (M_PI / 2)) {
		yAngle += (0.0014495849609375 * mouseY);
	}
	ly = sin(yAngle);
